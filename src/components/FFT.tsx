...
const FFT = forwardRef(
    (
      ...
      px-3 py-1 text-sm font-medium transition-all duration-300 rounded-lg
    ...
    `;
                ...
                case 'fullcandle':
                    return (
                        <div className="fixed inset-0 bg-highlight z-50 flex flex-col items-center justify-end">
                            ...
                            <div className="w-full max-w-4xl h-full flex justify-center pb-4">
        ...
        const plotData = useCallback(() => {
            ...
            const yMax = Math.max(...fftData.flat(), 1);
            ...
            // Y-axis labels shifted by topMargin
            ctx.fillStyle = axisColor;
            ctx.font = "12px Arial";
            ctx.textAlign = "right";
            ctx.textBaseline = "middle";
            for (let i = 0; i <= 5; i++) {
                const value = ((yMax * i) / 5).toFixed(3);
                const labelY = height - bottomMargin - (i / 5) * (height - bottomMargin - topMargin);
                ctx.fillText(value, leftMargin - 5, labelY);
            }
            ...
            ctx.textAlign = "center";
            ctx.fillText("Magnitude", -height / 2, topMargin - 10);
            ctx.restore();
